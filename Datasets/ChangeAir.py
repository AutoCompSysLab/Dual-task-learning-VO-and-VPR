...
import os.path
from .listdataset import TrainListDataset

def Changeair_Dataset(root, source_image_transform=None, target_image_transform=None, flow_transform=None,
    ...
    for dataset_name in dir_list:
        for data_type_name in data_type_list:
            ...
            for subdir in subdir_list:
                ...
                posefile = os.path.join(pose_root, dataset_name, dataset_name, data_type_name, subdir, 'pose_left.txt')
                image_list_ref=[]
                image_list_query=[]
                flow_list=[]
                mask_list=[]
                ...
                for i in range(len(flow_list)):
                    ...
                    train_list.append([[image_list_ref[i], image_list_query[i]], flow_list[i], mask_list[i], motion_list[i]])

    print('Loading train dataset at {}'.format(root))
